...
class aciDB:
    ...
    def login(self, uid, pwd, apic):
        ...
        try:
            ...
            print 'Exception Could not connect to APIC2: ', self.apic
            return '300'
            # raise
    ...
    def get_epg_node_table(self, tenant_name, epg_name):
        result = []
        ret_dict = {}
            ...
            for epg_dn in sorted(epg_dict.keys()):
                ...
                for port_name in sorted(port_dict.keys()):
                    if epg_dn in port_dict[port_name]['epg']:
                        if port_dict[port_name]['switch'] not in epgnodelist:
                            epgnodelist.append(port_dict[port_name]['switch'])
                        if 'regular' in port_dict[port_name]['mode']:
                            if 'regular' not in epgmodelist:
                                epgmodelist.append('regular')
                        if 'native' in port_dict[port_name]['mode']:
                            if 'native' not in epgmodelist:
                                epgmodelist.append('native')
                        if 'untagged' in port_dict[port_name]['mode']:
                            if 'untagged' not in epgmodelist:
                                epgmodelist.append('untagged')
                        if 'MIGRATION' in port_dict[port_name]['ipg']:
                ...
                if tenant_name == epg_dict[epg_dn]['tenant'] or tenant_name == None:
                    ...
                    row = [epg_dict[epg_dn]['tenant'], epg_dict[epg_dn]['app'], epg_dict[epg_dn]['name'],
                           ...
                           epg_dict[epg_dn]['ctx'], epg_dict[epg_dn]['bd'], ','.join(epg_dict[epg_dn]['bd_subnet']),
                           (','.join(epg_dict[epg_dn]['vlan'])).replace('vlan-', '', 10),
                           ','.join(epg_domain_names), migration, ','.join(epgmodelist), ','.join(epgnodelist)]
                    result.append(row)

            result_headers = 'Tenant, App Profile, EPG, Description, Context, Bridge Domain, Anycast GW, ' \
                             'Vlan, Domain, Status, Modelist, Nodelist'
            ret_dict[result_headers] = sorted(result)
            ...
            for epg_dn in sorted(epg_dict.keys()):
                if tenant_name == epg_dict[epg_dn]['tenant'] or tenant_name == None:
                    ...
                    if epg_name == epg_dict[epg_dn]['name']:
                        ...
                        if ret_dict == {}:
                            ret_dict = {
                                '01, EPG': epg_dict[epg_dn]['name'],
                                '02, Tenant, App Profile, EPG, Description': [],
                                '03, Context, Bridge Domain, Anycast GW, Static, Domain, Modelist, '
                                'Nodelist': [],
                        ...
                        ret_dict['02, Tenant, App Profile, EPG, Description'].append(
                            [epg_dict[epg_dn]['tenant'], epg_dict[epg_dn]['app'], epg_dict[epg_dn]['name'],
                             epg_dict[epg_dn]['descr']])
                        ret_dict['03, Context, Bridge Domain, Anycast GW, Vlan, Domain, Status, Modelist, '
                                 'Nodelist'].extend(
                            [[epg_dict[epg_dn]['ctx'], epg_dict[epg_dn]['bd'],
                              '\n'.join(epg_dict[epg_dn]['bd_subnet']), '\n'.join(epg_dict[epg_dn]['vlan']),
                              '\n'.join(epg_dict[epg_dn]['domain']), migration,
                              '\n'.join(epgmodelist), '\n'.join(epgnodelist)]
    ...
    def get_bd_table(self, tenant_name, bd_name):
        ...
        for bd_dn in sorted(bd_dict.keys()):
            ...
            for epg_dn in bd_dict[bd_dn]['epg']:
                if tenant_name == None:
                    bd_epg_list.append(epg_dn.split('uni/tn-')[1].split('/')[0] + '/' + epg_dn.split('/epg-')[1])
                    bd_list = bd_dict.keys()
        ...
        if bd_name == None:
            for bd_dn in bd_list:
                if bd_name == bd_dict[bd_dn]['name'] or bd_name == None:
                    if bd_dict[bd_dn]['tenant'] == 'common':
                        bd_dict[bd_dn]['name'] = '*' + bd_dict[bd_dn]['name']
                    else:
                        bd_dict[bd_dn]['name'] = bd_dict[bd_dn]['tenant'] + '/' + bd_dict[bd_dn]['name']
                    ...
                    result.append(row)
            result_headers = 'Bridge Domain, Description, Context, Route, ARP, Unicast, Subnet, EPG, L3out'
            ret_dict[result_headers] = sorted(result)
            ...
            for bd_dn in bd_list:
                if bd_name == bd_dict[bd_dn]['name']:
                    if ret_dict == {}:
                        ret_dict = {
                            '01, Bridge Domain': bd_dict[bd_dn]['name'],
    ...
    def get_contract_table(self, tenant_name, contract_name):
        ...
        for contract_dn in contract_list:
            ...
            if contract_name == contract_dict[contract_dn]['name'] or contract_name == None:
                ...
                row = [contract_dict[contract_dn]['name'], contract_dict[contract_dn]['sg'],
                ...
                result.append(row)
        result_headers = 'Name, Service Graph, Source, Destination, Entries, Stateful, Type, Prot, Ports, Direction'
        ret_dict[result_headers] = sorted(result)
        return ret_dict
        ...
        if ipg is None:
            for ipg_name in ipg_dict.keys():
                ...
                if type == ipg_type or type == None:
                    ...
                    if ipg_type == 'accbundle-link': ipg_type = 'pc'
            ...
            if ipg in ipg_dict.keys():
                ...
                if ipg_dict[ipg_name]['type'] == 'accbundle-link': ipg_type = 'pc'
        ...
        for vlan_name in vlan_dict.keys():
            ...
            if vlan:
                ...
                if vlan == str(vlan_dict[vlan_name]['id']):
                    row = [int(vlan_dict[vlan_name]['id']), vlan_name, '\n'.join(vlan_dict[vlan_name]['aep']),
                           '\n'.join(vlan_dict[vlan_name]['interfaces']), '\n'.join(vlan_dict[vlan_name]['intf_descr']),
                           '\n'.join(vlan_dict[vlan_name]['bd']), '\n'.join(vlan_dict[vlan_name]['epg_tenant']),
                           '\n'.join(vlan_dict[vlan_name]['epg_list']), '\n'.join(vlan_dict[vlan_name]['mode'])]
                    result.append(row)
                ret_dict[result_headers] = sorted(result)
        ...
        for intf_name in intf_dict.keys():
            ...
            for epg_dn in intf_dict[intf_name]['epg']:
                if '/epg-' in epg_dn:
                    ...
                    epg_list.append(epg_dn.split('uni/tn-')[1].split('/')[0] + '/' + epg_dn.split('/lDevVip-')[1])
                else:
                    epg_list.append(epg_dn)
                ...
                if endpoint in str(endpoint_dict[endpoint_name]['ip']) or \
                    ...
                    for i, epg in enumerate(epg_list):
                        ...
                        if endpoint_dict[endpoint_name]['type'][i] == 'accbundle-link':
                            endpoint_dict[endpoint_name]['type'][i] = 'pc'
                        ...
                        for endpoint_ip in endpoint_ips:
                            if endpoint in endpoint_ip or endpoint.lower() in endpoint_name.lower():
                                row = [netaddr.IPAddress(endpoint_ip),
                    ...
                    for i, epg in enumerate(epg_list):
                        ...
                        for endpoint_ip in endpoint_ips:
                            row = [netaddr.IPAddress(endpoint_ip),
                                   ...
                                   ','.join(endpoint_dict[endpoint_name]['interfaces'][i])]
                            result.append(row)
        result_headers = 'ip, mac, epg & external-egp  , encap, type, Interfaces'
        ret_dict[result_headers] = sorted(result)
        return ret_dict
...
def get_data():
    c = [('ab:ac:ac:ac:ad:ad', '1.1.1.1', 'tenant', 'app', 'epg', 'interface', '0000-00-00 00:00:03',
          '0000-00-00 00:00:44'),
...
apic_dict = {
    'sandboxapicdc.cisco.com': ['box1', 'sandboxapicdc', 'sandboxapicdc.cisco.com', 'sandboxapicdc.cisco.com'],
...
class aciapp(BaseView):
    ...
    def show_bd(self):
        try:
            ...
            if bd_name:
                ...
                row_url = ['href=bdtable?token=' + token + '&grep=''&bd_name=', '', '', '', '', '', '',
                           'href=epgtable?token=' +
                           token + '&grep=''&epg_bd=', '', '']
                return render_template('main5.html', formaction=formaction, formname=formname, apic=apic, token=token,
                                       username=login, url=url, tableheader=data['header'], tablerows=data['row'],
    ...
    def show_endpoint(self):
        try:
            ...
            endpoint = None
            grep_list = request.args.get('grep')
            result = aci.get_endpoint_table(endpoint)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_contract(self):
        try:
            ...
            grep_list = request.args.get('grep')
            result = aci.get_contract_table(tenant_name, contract_name)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_port_stat(self):
        try:
            ...
            port = None
            grep_list = request.args.get('grep')
            result = aci.get_port_stat_table(node, port)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_ipg(self):
        try:
            ...
            node = None
            grep_list = request.args.get('grep')
            result = aci.get_ipg_table(node, ipg_type, ipg_name)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_fex(self):
        try:
            ...
            node = None
            grep_list = request.args.get('grep')
            result = aci.get_fex_table(node, ipg_name)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_vlan(self):
        try:
            ...
            if vlan:
                row_url = ['href=vlantable?token=' + token + '&grep=''&vlan=',
                           'href=vlantable?token=' + token + '&grep=',
    ...
    def show_vlan_domain(self):
        try:
            ...
            domain = None
            grep_list = request.args.get('grep')
            result = aci.get_vlan_per_domain_table(domain)
            data = get_data3(result)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_lldp(self):
        try:
            ...
            node = None
            grep_list = request.args.get('grep')
            result = aci.get_lldp_table(node)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_node(self):
        try:
            ...
            node = None
            grep_list = request.args.get('grep')
            result = aci.get_switch_table(role, node)
            data = get_data3(result, grep_list)
            aci.logout()
            userlog.append(time.ctime() + ', ' + str(request.remote_addr) + ', ' + str(login) + ', ' + str(apic) + \
    ...
    def show_snapshot(self):
        try:
            ...
            return render_template('snapshottable.html', formaction=formaction, formname=formname, apic=apic,
                                   token=token,
                                   username=login, url=url, tableheader=data['header'], tablerows=data['row'])
        except:
            return redirect(url_for('aciapp.index', token=token))
